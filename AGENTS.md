...
2. **Root Directory (The Companion App)**: A Next.js web application utilizing Firebase Firestore for real-time Mobile-to-TV QR/Code pairing.
   - *Future Transition*: We are moving towards a Local LAN-based QR server running directly on the TV (NanoHttpd/Ktor) to avoid needing Firebase for basic pairing.
...
## Tech Stack
- **Native TV**: Kotlin, Compose for TV, Media3 ExoPlayer, Room, WorkManager.
- **Web**: Next.js, Tailwind, Firebase (for now).
